...
try.out <- try(data.list(community,environment,traits,dimids=c("sites","species")))
cat(try.out)
@
...
<<eval=TRUE>>=
dl <- dlcast(list(community,environment,traits),dimids=c("sites","species"),fill=c(0,NA,NA))
@
...
\subsection{Manual dimension matching with \code{match.dimids}} 
