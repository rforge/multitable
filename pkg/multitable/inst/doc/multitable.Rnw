...
\begin{tikzpicture} [
	...
	Y/.style={
		...
		minimum width=8cm,minimum height=12cm,
...
\section{The \code{multitable} model of data organization}

The \code{multitable} model of data organization is illustrated in Figure \ref{fig:model}.  In blue are the elements of the standard \R\ workflow: data frames; formulas; functions; and analyses.  The \code{multitable} package seeks to facilitate the use of such workflows with multiple-table data by creating tools (arrows and red boxes) for organizing and manipulating such data.  Once data lists are ready for analysis \code{multitable} provides tools for coercing data lists into data frames, thereby entering the standard \R\ workflow.
