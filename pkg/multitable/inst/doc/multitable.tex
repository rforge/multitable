...
On occasion, one may wish to iteratively coerce a sequence of data lists to data frames.  For example, in a randomization test one might loop over a number of random subscripts of a data list.  In such a case, each resulting data frame has the same structure (i.e. the same replication dimensions and variables).
\begin{Schunk}
\begin{Sinput}
> rand.interaction <- function(){
 	x <- xrep <- fake.community
 	b <- rep(0,100)
 	for(i in 1: 100){
 		xrep$abundance  <- 
 			x$abundance[sample(6),,]
 		b[i] <- lm(abundance ~ temperature * body.size,
 			xrep)$coefficient[4]
 	}
 	quantile(b,probs=0.025,0.975)
 }
\end{Sinput}
\end{Schunk}
...
\begin{Schunk}
\begin{Sinput}
> rand.interaction.mold <- function(){
 	x <- xrep <- fake.community
 	mold <- data.list.mold(x)
 	b <- rep(0,100)
 	for(i in 1: 100){
 		xrep$abundance  <- 
 			x$abundance[sample(6),,]
 		xrep.df <- as.data.frame(xrep, mold=mold)
 		b[i] <- lm(abundance ~ temperature * body.size,
 			xrep.df)$coefficient[4]
 	}
 	quantile(b,probs=0.025,0.975)
 }
\end{Sinput}
